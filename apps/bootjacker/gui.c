...
int8_t gui_msgbox(GUI_t *gui, const char* msg, enum msgbox_type_t type)
{
    ...
    {
        if (pgm_read_byte(&btn_strings[type][i]) != 0xFF)
        {
            btn_count++;
        }
    }
    ...
    for (uint8_t i = 0; i < btn_count; i++)
    {
        ...
        uint8_t btn_id = pgm_read_byte(&btn_strings[type][i]);
        // uint8_t btn_id = pgm_read_byte_far((uintptr_t)btn_strings + sizeof(btn_strings[0])*type + sizeof(btn_strings[0][0])*i);
        ...
        indirect_call(gfx_fill)(gui->gfx, 0x65bd);
        MODULE_CALL_THIS(gfx, rectangle, gui->gfx, region);
        if (btn_id > 7)
            continue;
        const char *s = (const char*)pgm_read_word((uintptr_t)strings + sizeof(strings[0])*btn_id);
        MODULE_CALL_THIS(gfx, fill, gui->gfx, BLACK);
        MODULE_CALL_THIS(gfx, textP, gui->gfx, ((display_region_t){x-3*my_strlen_P((uintptr_t)s), 148, 0, 157}), (uintptr_t)s);
    }
