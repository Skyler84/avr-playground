...
MODULE_FN_PROTOS(module, MODULE_FUNCTION_EXPORTS);
REGISTER_MODULE(module, MODULE_MODULE_ID, MODULE_FUNCTION_EXPORTS, MODULE_API_VER);

fn_ptr_t module_fn_lookup(uint16_t _id, moduleptr_t module_addr) {
  uint32_t fn = 0;
  if (module_addr == 0) {
    module_addr = (uintptr_t)&module + GET_MODULE_DATA_PTR_OFFSET();
  } else {
  }
  fn = (uint32_t)(module_addr + (uint32_t)offsetof(module_t, fn_table));
  if (fn == 0) {
    return NULL;
  }
  uint16_t module_type = pgm_read_word_far(module_addr + offsetof(module_t, type));
  uint16_t id;
  while ((id = pgm_read_word_far(fn + (uint32_t)offsetof(module_fn_t, id))) != 0x0000U) {
    if (id == _id) {
      return (fn_ptr_t)(pgm_read_word_far(fn + (uint32_t)offsetof(module_fn_t, fn)) + (module_type == MODULE_TYPE_MODULE?pgm_ptr_to_fn_ptr(module_addr):0));
    }
...
typedef void (*error_handler_t)(module_id_t, moduleptr_t*, uint16_t, fn_ptr_t *);
void dummy_handler(module_id_t, moduleptr_t*, uint16_t, fn_ptr_t *){}

void module_init_fns(fn_ptr_t fns[], module_id_t id, uint16_t fn_ids[]) {
  moduleptr_t module = module_find_by_id(id);
  error_handler_t error_handler = indirect_call(dummy_handler);
  if (module == (moduleptr_t)0) {
    error_handler(id, &module, 0, NULL);
  }
  for (uint8_t i = 0; fn_ids[i]; i++) {
    fns[i] = module_fn_lookup(fn_ids[i], module);
    if (fns[i] == NULL) {
      error_handler(id, &module, fn_ids[i], &fns[i]);
    }
  }
}
